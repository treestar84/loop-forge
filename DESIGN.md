...
### 6.2 큰 루프 프로토콜 v2 — 래더·패배 채굴·포트폴리오 (2026-07-29, GAP-11)

§6.1의 4단계를 다음 6단계로 확장한다(GAP-ANALYSIS-11 D5.5, ADR-0011~0013).
v1과의 차이는 세 가지다: **외부 앵커 래더가 손실 함수가 됐고**(R1 처치),
**패배가 기계가 읽는 자산으로 남으며**(R2 처치), **후보 생성이 단일 아이디어
직렬 시도에서 버킷 포트폴리오가 됐다**(R4 처치).

1. **웨이브 + challenge 계측**: `WaveConfig.challenge`로 피드백 앵커(L1 중수·
   L2 고수) 승률을 게이트 판정과 같은 리포트에 계측(채택 판정 불개입 —
   ADR-0012). 홀드아웃 앵커(L3)는 코드 수준에서 진입 차단.
2. **패배 채굴**: near-miss 구조화(기존) + `mineLosses`→LossReport(분기점
   대조·결정지점 불일치율·첫 분기 깊이) + 프로브 은행 갱신(`buildProbeBank`,
   분기 국면 영구 봉인 — 패배가 회귀 자산이 된다).
3. **설계 브리프 자동 생성**: `renderDesignBrief`(artifacts/design-brief.ts)가
   게임 요약·채택 이력·LossReport·프로브 통계·축 매트릭스를 조립. 근거 수준을
   머리에 명시하고 홀드아웃 수치는 fail-safe로 배제.
4. **포트폴리오 발주**: B1(기계 변형)~B5(모방) 버킷 비중(ADR-0013)에 따라
   후보 배치 생성 — B3(심층 설계)만 Opus급, B1은 LLM 0.
5. **프로브 필터 → 웨이브**: 프로브 일치율·판당 비용으로 배치를 상위 k로
   압축한 뒤 정규 다단 게이트(신규 시드 뱅크) 진입 — 채택 판정은 v1과 완전
   동일(특례 없음).
6. **수율 재배분 → 1로 복귀**: `reallocate`(하한 5%)로 버킷 비중 갱신 후
   `portfolio-state.json` 봉인. 종료 조건은 "축 매트릭스 소진 + challenge
   승률 정체 3라운드"이지 라운드 수 상한이 아니다. **초월 판정만은 홀드아웃
   앵커(L3) 상대 CI 하한 > 0.5로 별도 수행**한다.

ADR-0009(억지 반복 금지)의 적용 단위는 "같은 축의 수동 재시도"다 — B1의
기계 스윕은 수율로 자동 축소되므로 모순되지 않는다. 실증 기록과 실측 수치는
`docs/GAP-11-ROUNDS.md`가 정본이다.
