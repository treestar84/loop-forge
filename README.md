...
| **가장 극적인 반전** | 윙스팬에서 상대 AI 승률 76.9% → 24.3%(10판 중 8판 지던 게임 → 10판 중 8판 이기는 게임으로 역전) |
...
| **품질 보증** | 자동 테스트 833개 전부 통과, 타입 오류 0건(위 CI 배지가 매 커밋마다 이 사실을 재확인한다) |
...
**2. 새 전략을 만들고, 통계로 검증한다.** 새 전략 후보를 여러 개 만든 뒤, 좌석을
바꿔가며 반복 대국시키고 두 가지 통계 시험을 통과해야만 채택한다 — **SPRT**(순차
통계 검정: 차이가 확실하면 빨리 판정하고, 애매하면 판수를 자동으로 늘린다)와
**holdout**(학습 과정에서 한 번도 보지 않은 판으로 마지막에 재검증 — 특정
문제만 잘 푸는 "벼락치기"를 걸러낸다). 이 두 단계를 프로젝트 내부에서는
"큰 루프"(후보 생성)와 "작은 루프"(통계 검증)라고 부른다.
...
구성해 대결시키는" 능력(`fieldMix`)이 실제로 동작하는지 검증했다.
...
|---|---|---|---|
| 오목 | 0%(다섯 가지 접근 전부 실패) | 50.0%(200판 재확인 — 통계적 대등, 서두 참고) | 챗봇 AI의 판단 로직을 탐색 트리의 "우선순위 힌트"로 이식 — 그대로 흉내 내면 오히려 약해지고, 힌트로 살짝 얹을 때만 통했다 |
...
- **오목**: 탐색 예산을 2배로 늘리기·전략을 다른 방식으로 바꾸기·전술 판단을
  ...
  원인(루프 자체가 "챗봇 AI를 이겨야 한다"를 목표로 삼은 적이 없었다)을
  찾아 고친 뒤 통계적 대등까지는 왔지만, 그 위에 무엇을 더 얹어도(동점 국면
  보정·추가 힌트 결합) 유의한 우위로는 안 바뀌었다 — "이번 방법으로는 여기가
  한계"라는 사실도 그대로 공개해뒀다.
