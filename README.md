...
<div align="right"><a href="#toc">↑ 목차</a></div>
...
> 선행 프로젝트(웹 티추 NPC 하네스: 후보 4,100개 · 16만+ 판 · 과적합 발견과
> 아키텍처 재설계)에서 실제로 터졌던 사고 — 좌석 편향 +15%p, 고정 시드 과적합,
> ±7.9%p 노이즈 오채택, 관찰 오염 — 를 채점 축과 게이트 규칙으로 일반화한 것이
> 이 설계의 근거다.
...
> 구성해 대결시키는" 능력(`fieldMix`)이 실제로 동작하는지 검증했다. 자세한
> 내용은 [docs/GAP-ANALYSIS-10.md](./docs/GAP-ANALYSIS-10.md).
...
실제로 채택한 전략을 서로 맞대결시켰다. **숫자가 낮을수록 Loop Forge가 이긴
것**이다. 즉흥 설계 vs 체계적 검증의 차이를 정면으로 비교한다.
...
[docs/GAP-11-ROUNDS.md](./docs/GAP-11-ROUNDS.md) 라운드 기록), 같은 절차를
세 게임에 적용한 결과:
...
<details>
<summary><b>📜 이전 기록 보기</b> — 2026-07-21~28 벤치마크(문맥이 달라 위 표와 직접 비교 금지, 참고용)</summary>
...
| 오목 | 97.1% | 100% (v5, N=1,300 — 5개 카드 전부 시도) | 미역전 당시 상태(2026-07-28) — 그 이후 진행은 위 "지금 가장 최신" 절 참고 |
| 도미니언 | 95.3% (v2) | (기준선 유지, near-miss 내부 승률은 0.275→0.400으로 개선 추세) | IS-MCTS 후보가 회귀 게이트에서 챔피언에 계속 근접 중, 당시 채택 미달 — 이후 진행은 위 절 참고 |
...
**그래프로 보면** (막대가 짧을수록 Loop Forge 우세, 2026-07-28 시점 스냅샷):
...
오목       현재  ███████████████████████████ 100.0%  (5가지 시도 전부 실패, 원인 공개 — 이후 갱신은 위 절 참고)
도미니언   현재  █████████████████████████░░ 95.3%   (관문이 열세 후보 정직 차단 — 이후 갱신은 위 절 참고)
...
"성공만 보여주는" 도구가 아니다. 실제로 시스템이 스스로 실패를 인정한 사례들:
