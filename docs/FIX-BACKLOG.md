...
| — | **registry 조립 시맨틱 재설계**: composeBot이 MCTS 계열 플래그에서 base를 버리는 관례 때문에 다중 채택이 "마지막 플래그 단독"과 동일해짐(오목 v6 실측 — opening6 소실, GAP-11 Phase 4-A). **2차 실증(2026-07-30)**: 도미니언 v4에서 복수 채택 5개 중 배열 마지막(opusCloneDominion)만 생존 — chapelEconomyV2(42.5%)의 진짜 개선이 죽은 코드화. 잠정 규칙 2건: ① 탐색 계열 후보는 단일 자립 플래그 ② 복수 채택 시 challenge 최고 성적 후보를 지배자(배열 마지막)로 명시 배치. 근본 처치(조립 시맨틱에 앙상블/우선순위 표현)는 별도 결정 필요 | 4 | 미착수(잠정 규칙으로 운영 중) |
