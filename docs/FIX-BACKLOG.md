...
| E2-B | 장기·아발론·카탄 — 채택 후보 0개 상태부터 해결(GAP-11 적용 전 선행 과제). 게임별 A8 도메인 재설계 카드는 GAP-12 §7 표 참고 | 진행 중(장기 **완료** — janggiPieceSafetyMobility adopted v1→v2, holdout 95.0%[85.0-100%]. 아발론 **1차 실패**(2026-08-05) — avalonSuspicionTracking(투표이력 베이지안 의심도, 관찰에 이력 존재 확인 후 순수함수로 구현) smoke 20.0%로 SPRT 기각. 원인 가설 3개 기록(참가 페널티가 무고한 서번트를 오염시킴·미션 5회뿐이라 신호 축적 부족·멀린/이블은 이미 정보가 있어 무의미). **2차도 실패**(원인 분리 3후보 전부 failed, 가설①·③ 기각, 의심도 축적 메커니즘 축 소진 — docs/GAP-11-ROUNDS.md "아발론 A8 2차 시도" 참고). 카탄 **1차 실패**(2026-08-06) — catanScarcityWeightedTrade(건설+은행거래 통합 필요벡터 평가함수, 플레이어간 거래 부재 확인 후 은행거래로 재해석) smoke 4.5%로 SPRT 조기 기각. 처리량 문제 없음(0.99x). 다음 카드 3개 제시(build/trade 분리, "거래 자체를 안 하는 게 최선" 가설 검토, 완전 평가함수형 전환) — E2-B 그룹(장기·아발론·카탄) 1차 스윕 종료: 장기만 채택 성공. **후속(2026-08-07)**: 장기를 GAP-11 정규 편입 — 극성 판정(heuristic 27.8%, L2 우세, 옛 78.0% 기록과 정합) → L1/L2 게이트 통과(L1 초안이 너무 강해 top-K/가중치 1회 재조정) → L3(ADR-0015 보정 게이트, 수비 우선+병사 조직적 전진, fast-pass 30.9%) → 트리거 67.3%→확증 76.8%[72.5-80.9%]→L3 홀드아웃 80.5%[74.0-86.0%] 전부 통과 → **초월 확정(3호)**. 브리프 자신이 예상한 "v2가 L2조차 못 넘을 가능성"을 뒤엎은 결과 — 상세는 docs/GAP-11-ROUNDS.md "장기 GAP-11 편입". **카탄 2차도 실패**(2026-08-09) — catanPipWeightedBuild(bankTrade 완전 배제, build 유형 우선순위는 heuristic과 동일, 유형 내 좌표 선택만 heuristicPick의 buildInitial pip 평가함수로 교체) smoke 25.0%(scoreDiff +0.289, 15블록)로 minWinRate(0.53) 미달, near-miss 없음. C6 4/5 distinct(신규 플래그 no-op 아님), C4 처리량 0.99x(문제 없음), tsc/test 회귀 없음(69 suites/964 tests). 1차(6.3%)보다 개선됐으나(bankTrade 배제 효과는 있었음) 여전히 크게 부족 — "거래 자체가 순손실" 가설은 반증에 가까움(거래 완전 제거해도 여전히 크게 짐), "휴리스틱 평가함수형 전환"도 이번엔 효과 없음. 다음은 로버 이동/discard 등 미탐색 결정지점이나 C5가 드러낸 좌석 편향(0.485) 자체를 먼저 진단하는 방향 제안 — 상세는 docs/GAP-11-ROUNDS.md "카탄 A8 2차 시도(건설/거래 분리)". **카탄 3차도 실패**(2026-08-09) — catanProductionDenialRobber(moveRobberAndSteal 결정지점, denialScore=selfTouchesHex?-hexValue:hexValue로 자해 회피+고생산 헥스 차단, tie-break는 target 존재→리더 순) smoke 20.0%(scoreDiff +0.028, 15블록, drawRate 0%)로 minWinRate(0.53) 미달, near-miss 없음. C6 5/6 distinct(신규 플래그 no-op 아님), C4 처리량 0.95x(문제 없음, base보다 근소하게 빠름), tsc/test 회귀 없음(69 suites/964 tests). 1차(6.3%)·2차(25.0%)에 이어 완전히 다른 결정지점(로버)을 개선했음에도 20.0%로 유사한 범위에 수렴 — bankTrade·build·robber 세 축 모두 20~25%대 실패로 수렴한다는 것은 개별 결정지점 국소 개선이 아니라 구조적 요인(유력 후보: C5가 3라운드 내내 동일하게 재현한 좌석 편향 bias=0.485, seatWinRates=[0.595,0.170,0.115,0.110])이 원인일 가능성을 시사. 카탄 채택 후보 0개 상태 3라운드 연속 — 다음은 좌석 편향 자체의 진단을 우선 제안(discard는 차순위 후보), 메인 루프 최종 판단 필요 — 상세는 docs/GAP-11-ROUNDS.md "카탄 A8 3차 시도(로버 결정지점)" |
...
| E5 | MCTS 옵션 필드 조합 계약 명시화 | **완료**(2026-08-07 — `validateMctsOptions` 순수 진단과 9개 옵션 조합/우선순위 표 추가; 기존 MCTS·IS-MCTS 런타임은 미배선으로 보존) |
| E6 | no-op 승격 억제 — 승격 후보의 flags가 부모 버전과 완전히 동일하면 새 버전을 만들지 않고 기존 버전에 `sourceWaveId`만 덧붙이는 처치(하스스톤 2회전에서 v4=v3 내용 동일로 실측, `docs/GAP-11-ROUNDS.md` 하스스톤 2회전 판정) | **완료**(2026-08-08) |
| E7 | 포트폴리오 수율 재배분이 "채택 여부"만 보고 "새 정보량"은 못 본다 — 하스스톤 2회전에서 B1이 이미 소진(우 소진: priorWeight 곡선 양옆 확인 완료)됐는데도 수율 계산상 80% 배분을 받음. 축 소진 여부를 수율 지표에 반영하는 처치 필요 | **완료**(2026-08-08 — wave adopted라도 `assembleFlags`에서 최종 배제된 이번 라운드 후보는 버킷 수율 adopted에서 제외) |
...
| E9 | **확증 N의 검정력 부족** — 오목 v9의 같은 후보를 N=100/200으로 세 번 측정하는 동안 점추정은 매번 53~55%로 일관됐는데 CI 하한만 두 번 정확히 0.5 문턱(49.8%) 근처에서 걸렸다. N=1000으로 올리자 CI 폭이 ±3.4%p→±1.5%p로 좁아지며 하한이 52.18%로 확실히 넘어 초월이 확정됐다(`docs/GAP-11-ROUNDS.md` "오목 v9 대규모 확증"). 즉 근접실패의 원인이 후보 설계 부족이 아니라 **확증 표본 크기의 검정력 부족**이었던 사례 — 다른 게임에서 같은 문턱 근접실패가 반복되면 재설계 전에 검정력부터 의심해야 한다는 원칙을 성문화할 필요 | **완료**(2026-08-08) — `docs/TROUBLESHOOTING.md` §13로 성문화: 같은 후보가 2회 이상 CI 하한만 문턱에서 걸리면 새 설계 전에 훨씬 큰 N(5~10배)으로 먼저 재확인하라는 절차. near-miss 반복 전반에 적용 |
| E11 | **FFA(3인 이상) 게임의 promotionMinWinRate가 identityCenter로 스케일되지 않고 2인 게임 전제(0.53)로 고정돼 있던 결함** — 카탄(playerCount=4)·아발론(playerCount=5)의 반복된 "실패" 판정이 전략 설계 부족이 아니라 이 임계값 결함 때문이었을 가능성. `kernel/blueprint.ts`의 `promotionMinWinRate`를 `identityCenter + 0.03`으로 수정해 2인 게임은 byte-identical 유지, FFA 게임은 identityCenter 기준으로 스케일되도록 고쳤다. 카탄의 기존 3개 카드를 보정된 기준(minWinRate=0.28, SPRT p0=0.25/p1=0.35)으로 재판정한 결과: `catanScarcityWeightedTrade`는 여전히 smoke failed(3.3%, scoreDiff -2.017) — 이 카드 자체는 보정과 무관하게 여전히 나쁨. `catanPipWeightedBuild`(smoke 33.3%→prune 35.0%)와 `catanProductionDenialRobber`(smoke 28.3%→prune 30.0%)는 3라운드 내내 failed였다가 이번엔 **near-miss**로 바뀜(smoke 통과, prune에서 미보정 cross-game 상수 minScoreDiff=5에 막힘 — scoreDiff 0.38~0.52로 부족). 셋 다 holdout 미도달, registry v1 그대로(승격 없음) — 진단은 맞았지만 채택까지는 이어지지 않았다, 있는 그대로 기록. 상세는 `docs/GAP-11-ROUNDS.md` "카탄 promotionMinWinRate FFA 보정 및 재판정". **아발론도 같은 결함의 영향을 받았을 가능성이 있으나 이번 위임 범위 밖 — 다음 트랙 후보로 남김**(아발론 재판정은 causal-isolation 2차 라운드가 후보 3개를 동시에 테스트한 복잡한 구조라 별도 설계 브리프가 필요함) | **완료**(2026-08-09) — 코드 수정 완료, 카탄 재판정 완료. 다음 후보: 카탄 near-miss 2건에 P6 스타일 `recommendedMinScoreDiff` calibration 배선(scope 밖으로 미룸), 아발론 재판정(scope 밖) — **[2차 재판정 추가, 2026-08-10]** 위 "다음 후보"로 남겨둔 calibration 배선을 실제로 연결해봤다. 이건 새 버그가 아니라 1차 재판정 러너(`catan-a8-rejudge.ts`)의 배선 누락이었다 — `measureNoiseFloor`로 `blockStdDev`/`scoreDiffStdDev`를 이미 계산해 두었으면서 `assembleWaveConfig` 호출에 3번째 인자(calibration)로 넘기지 않아 P6 보정 메커니즘이 무시되고 범게임 폴백 `minScoreDiff=5`가 그대로 쓰였다. 신규 러너 `catan-a8-rejudge2.ts`(구조 동일, 카드 3개 그대로, calibration 인자만 추가)로 재측정: 보정된 `minScoreDiff`는 실제로 5와 달라졌다(≈2.6e-16, 이번 측정 조건에서 노이즈 표준편차가 0으로 나와 사실상 0에 수렴 — 배선 자체는 정상 동작 확인됨). 결과: `catanScarcityWeightedTrade`/`catanProductionDenialRobber`는 여전히 smoke failed. `catanPipWeightedBuild`는 1차보다 한 단계 더 진출(screen→smoke→prune 통과, holdout까지 도달)했지만 holdout winRate 0.250 < minWinRate 0.28로 최종 **screened**(near-miss가 아님 — winRate 자체 미달, scoreDiff 단독 부족이 아니므로). registry v1 그대로, 채택 없음. 상세는 `docs/GAP-11-ROUNDS.md` "카탄 재판정 2차 — minScoreDiff 보정 배선". — **[E9 대규모 재확증 추가, 2026-08-10]** `docs/TROUBLESHOOTING.md` §13(E9 원칙)을 적용해 재판정 2차의 근접실패(holdout 25.0% vs 문턱 28.0%, 5블록뿐)를 재설계 없이 표본만 10배(5→50블록) 늘려 재확인했다. 신규 러너 `catan-a8-pipweightedbuild-large-confirm.ts`(구조 동일, `catanPipWeightedBuild` 단일 후보만, 새 시드 뱅크 1,020,000대). 결과: prune 33.0%(scoreDiff 0.385, 50블록), holdout 33.0%(scoreDiff 0.420, 50블록) — 둘 다 문턱(28.0%) 안정적으로 통과, 5블록 표본의 25.0%는 노이즈였음이 확인됨. **`runs/catan/registry.json`에 `v2`(flags=[catanPipWeightedBuild]) 실제 등록 — 카탄 사상 최초 채택.** 웨이브 러너가 승격까지 자동 수행(수동 절차 불필요). 상세는 `docs/GAP-11-ROUNDS.md` "카탄 catanPipWeightedBuild E9 대규모 재확증". — **[아발론 진단·재판정 추가, 2026-08-11]** 위에서 "다음 트랙 후보"로 남겨둔 아발론을 실제로 코드까지 확인해보니 **카탄과 구조적으로 다른 결함**이었다 — 단순 재적용 불가. 아발론은 `hiddenTeamStructure: true`(진영이 숨겨져 있어 `spec.teams`를 정적 선언 못 함)라 `classification.identityCenter`가 naive `1/playerCount=0.2`로 계산되는데, `score.ts`의 `scoreC5`는 이미 이 게임 부류에서 naive 1/N이 부당하다는 걸 알고 실측 `meanWinRate`로 대체하는 예외(`identityFairnessExempt`)를 갖고 있었지만 `blueprint.ts`의 `promotionMinWinRate` 계산은 이 예외를 전혀 반영하지 않았다. 게다가 그 예외가 쓰는 실측치(C5의 random 항등 자기대국 0.237)조차 promotionMinWinRate가 진짜 필요로 하는 값이 아니었다 — 필요한 건 **heuristic 항등** 자기대국 실측치(카탄 성공 사례가 정확히 이 방식을 씀)였다. 해결: `BlueprintCalibration`에 `measuredIdentityCenter?: number` 오버라이드 필드를 신설해 호출자가 실측치를 우선 제공할 수 있게 하고(안 넘기면 기존 `classification.identityCenter` 폴백, 카탄 등 기존 호출자 byte-identical), 신규 러너 `avalon-a8-rejudge.ts`로 아발론 heuristic 항등 자기대국을 실측(0.207 — naive 0.2, C5의 random 항등 0.237과 셋 다 다름을 확인)한 뒤 `measuredIdentityCenter=0.207`/`blockStdDev`/`scoreDiffStdDev`를 모두 `assembleWaveConfig`에 배선해 기존 4개 전략(`avalonSuspicionTracking`/`VoteOnly`/`ProposalOnly`/`GoodOnly`, round1/round2 전부 failed)을 재설계 없이 재판정. 결과: 보정된 문턱(minWinRate=0.237)에도 불구하고 4개 후보 전부 `screened`(`avalonSuspicionVoteOnly`가 holdout까지 진출했으나 winRate 0.200으로 문턱 미달) — registry v1 그대로, 채택 없음. 카탄과 달리 보정폭이 작고(naive 0.2→heuristic 0.207, +0.007) 후보들의 실력 자체가 문턱과 거리가 있어 판정이 뒤집히지 않았다 — 정직한 무반전 결과. 상세는 `docs/GAP-11-ROUNDS.md` "아발론 hiddenTeamStructure 기준선 진단·재판정". |
