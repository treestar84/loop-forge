...
| — | **registry 조립 시맨틱 재설계**: composeBot이 MCTS 계열 플래그에서 base를 버리는 관례 때문에 다중 채택이 "마지막 플래그 단독"과 동일해짐(오목 v6 실측 — opening6이 조립에서 소실, GAP-11 Phase 4-A). 임시 규칙: 탐색 계열 후보는 단일 자립 플래그로 설계. 근본 처치(조립 시맨틱에 앙상블/우선순위 표현 도입)는 별도 결정 필요 | 4 | 미착수(2026-07-29 등재) |
