...
## 10. S4·S5 실행 기록 (Phase D, 2026-08-03, 메인 루프 직접)

- `docs/ONBOARDING-PLAYBOOK.md` 신설 — 준비물 자가진단(범위 3문항) →
  1단계 진단 → 2단계 스캐폴드 → 3단계 채점 루프("1회로 끝나지 않는 게
  정상"을 명시) → 4단계 첫 웨이브(실패 verdict도 온보딩 성공) 순서로,
  각 단계에 명령·복붙 프롬프트·통과 수치(§2 게이트표의 해당 행)·흔한
  실패 처방(실측 사례 인용: 스플랜더 데드락, C7 60 캡 정상 등)을 담았다.
  수치의 정본은 코드이고 문서는 §2 표만 가리킨다(§5 드리프트 리스크 대응).
- README "내 게임에 적용하기" 교체 — 기존 5단계 프롬프트 표를
  `npm run onboard` 여정 + 게이트 요약표 + 플레이북 링크로 대체. v1의
  과장 두 건 정정: "이미 봇(NPC)이 있는 게임" → "기존 봇 필요 없음"(기준선
  봇은 온보딩이 새로 만든다), "포트폴리오 프로토콜대로 웨이브" → 첫
  온보딩은 단순 웨이브까지가 완료 조건임을 명시.
- CLAUDE.md 문서 지도에 플레이북 추가, FIX-BACKLOG S0~S5 완료 갱신.
- 다음: Phase E(S-E) — 이 설계 문서를 제공하지 않은 신선한 에이전트가
  플레이북만으로 신규 미니 게임을 완주하는 종단 판정 실험.

## 11. S-E 실행 기록 — 종단 판정 실험 (Phase E, 2026-08-03)

이 설계 문서·핸드오프·DESIGN.md를 읽는 것이 금지된 신선한 에이전트가
"커넥트포(7x6 중력 4목)를 만든 취미 개발자" 역할로, README와 플레이북(및
그 둘이 명시적으로 링크한 문서·CLI 출력)만으로 온보딩을 시도했다.

**결과: G1→G4 완주 성공.**

| 게이트 | 결과 | 수치 |
|---|---|---|
| G1 진단 | 통과 | 판정 "구현 필요", P-Score(추정) 75%, blocker로 무작위성 미시드(Math.random) 정확히 지목 |
| G2 스캐폴드 | 통과 | TODO(onboard) 18개 → 0개, tsc 0에러 |
| G3 채점 | 통과(4회 반복) | C0~C6 전부 100, C7만 blocker(원본 리플레이 없음 — 정상) |
| G4 첫 웨이브 | 통과 | 후보 1개(edgePreference) screen 탈락 — "후보 전부 탈락해도 온보딩 성공"이 실측 재현됨 |

P-Score 75%(추정) 대비 완주 후 유효 C-Score(C7 제외)는 100 — 추정이
보수적이었다(감점 요인이던 미시드 무작위성이 어댑터 재구현으로 해소).
산식 보정 입력으로 기록해 둔다: P2 감점은 "원본의 결함"이지 "어댑터의
결함"이 아니므로, 리포트에 "이 감점은 어댑터 구현으로 해소 가능"임을
구분 표기하는 것이 다음 개선 후보.

**마찰 로그가 잡아낸 결함 (즉시 처치 대상)**:

1. **(심각) C-Score 표시 모순** — C0~C6=100·C7만 blocker인 통과 상태에서
   화면에 "실측 C-Score: 0%"가 뜨고, 동시에 "다음 행동: wave"로 통과를
   인정. 사람이 없으면 "0%인데 진행?"에서 잘못된 중단 판단 위험. 처치:
   표시·저장 점수를 "C7 제외 유효 점수"로 변경 + C7 상태는 별도 경고 줄
   (게이트 판정 로직 자체는 정상 — 표시 규칙만의 결함).
2. **G2에서 타입 계약 위치 미안내** — TODO를 채우려면 결국
   `src/contract/types.ts`를 스스로 찾아야 했다. 처치: 스캐폴드 골격
   doc comment·scaffold 출력·플레이북에 정본 위치 안내 추가.
3. **C1↔C6 축 간섭 미경고** — 시드 다양성(C1)을 고치는 오프닝 변형이
   전략 플래그(C6)를 no-op으로 만드는 트레이드오프를 시행착오로 발견.
   처치: 플레이북 "흔한 실패" 표에 항목 추가.
4. (경미) `wave`가 실행 명령만 출력하고 실제 spawn하지 않음(§1 설계와
   불일치), summary.md에 스테일 문구 잔존. 처치: spawn으로 변경, 원인 조사.

**판정**: "플레이북만으로 완주 가능한가" — **절차·순서·통과 기준은 예,
단 코드 계약 세부는 아니오**(타입 정본 열람 필요). 이는 설계가 예고한
한계("코드 작성 자체는 코딩 에이전트 몫")와 정합하지만, 위 결함 1은
사람/에이전트 모두를 오판시킬 수 있는 실제 구멍이라 즉시 수정한다.
실험 산출물(커넥트포 어댑터·runs)은 실험 전용 가상 게임이므로 저장소에
병합하지 않고 폐기(마찰 로그만 이 절에 남긴다).
