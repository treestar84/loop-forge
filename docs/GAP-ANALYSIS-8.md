...
## 4.8 오목 C열 역전 — 전술 프리체크(P7, 마지막 카드) 결과 (2026-07-26)

§4.6의 "접근 자체의 한계" 결론에 대한 마지막 반례 시도: Opus 즉흥봇의 우위
요소 중 "즉승 수/즉방어 수 최우선"만은 게임 지식이 아니라 게임 중립적으로
일반화 가능하다(체스 엔진의 전술 프리체크와 동일한 발상 — `applyChoice`+
`getOutcome`만으로 어떤 게임에서도 "이 수를 두면 즉시 이기는가"(1-ply)/
"이 수를 두면 상대가 즉시 이기는 응수가 있는가"(2-ply)를 계산할 수 있다).

`search/mcts.ts`의 `MctsConfig`에 `tacticalDepth?: 0 | 1 | 2`(기본 0=비활성,
미지정 시 기존 동작 완전 불변 — 신규 8개 테스트로 핀)와
`tacticalBranchCap?: number`(2-ply 검사 대상 수 제한, O(legal²) 비용 관리용)를
추가했다. depth≥1은 루트 각 합법수에 `applyChoice`+`getOutcome`을 적용해
즉승 수를 찾으면 MCTS 시뮬레이션 없이 그 수를 즉시 반환하고, depth≥2는
즉승 수가 없을 때 각 후보를 적용한 뒤 상대 차례에서 상대의 즉승 여부를 검사해
"안전하지 않은" 후보를 제외한 나머지로 루트 legal을 좁힌다(안전한 후보가
0개면 원래 legal 전체로 폴백 — 강제 선택이 아니라 안전망).

새 플래그 `mcts2-s256-tactical`(`shared/gomoku-mcts-flag.ts`): 시뮬레이션·
uctC·rolloutCount·rolloutFactory(챔피언 롤아웃)는 `mcts2-s256-cr`과 동일,
차이는 `tacticalDepth: 2`뿐. 비용 실측(scratch, nice -n 10, 단독 프로세스,
3판): vs heuristic ≈252ms/block, vs v5 챔피언(내부 기준선) ≈1,607ms/block —
`mcts2-s256-cr`(88ms) 대비 비싸졌지만(depth-2 검사가 매 결정마다 O(legal²)
`applyChoice`/`getOutcome` 호출을 추가하기 때문) 여전히 웨이브 예산 안에
드는 수준이라 `tacticalBranchCap`은 적용하지 않았다(전수 검사).

**Opus봇 상대 진단 결과(N=100, 게이트 없는 순수 집계, 먼저 실행)**:
`mcts2-s256-tactical` vs Opus봇 = **승률 0.0%(CI [0.0%, 0.0%]), 100블록
전패** — §4.6의 3개 후보(mcts2-s512-hr/mcts2-s256-cr/mcts2-s512-cr)와
동일한 0/100. **유의미한 개선 없음 → 웨이브·벤치마크(작업 3)는 생략**
(억지 투입 금지, 작업 지시서의 명시적 조건에 따름).

**원인 추정**: 전술 프리체크가 실제로 작동한다는 것은 단위 테스트(즉승 수
즉시 반환, 상대 즉승을 허용하는 수 배제, 결정론, 안전 후보 0개 시 폴백)와
v5 챔피언 상대 결과(내부 기준선을 여전히 압도)로 확인된다 — 즉 이 프리체크
자체는 game-neutral하게 정확히 동작한다. 그런데도 Opus봇을 못 이기는 이유는
§4.6이 이미 짚은 Opus봇의 우위가 "즉승/즉방어 최우선" **+** "다방향 위협
스코어링"의 결합이라는 데 있다 — 후자(예: 열린 3을 만들어 두 방향에서 동시에
승리 위협을 거는 포크, 또는 아직 "다음 한 수로 즉승"은 아니지만 막지 않으면
2~3수 뒤 확정승이 되는 전개형 위협)는 1-ply/2-ply의 리터럴 즉승/즉패
판정으로는 전혀 포착되지 않는다. 프리체크는 "당장 지는 수"만 걸러낼 뿐, 오목
승부를 실제로 가르는 "상대보다 먼저 포크를 만드는" 능력은 여전히 MCTS의
얕은 트리 탐색(챔피언 롤아웃 256/512 시뮬레이션)에 맡겨져 있고, 그 탐색
자체가 Opus봇의 즉석 다방향 위협 스코어링만큼 깊거나 정확하지 못하다.

**P7 결론(이번 라운드 오목 C열 카드 소진)**: 예산 상향(§4.6 mcts2-s512-hr),
롤아웃 정책 교체(§4.6 mcts2-s256-cr/s512-cr), 전술 프리체크(§4.8
mcts2-s256-tactical) — 세 갈래 접근이 모두 Opus봇 상대 0%로 귀결됐다. 남은
가설(다방향 위협 스코어링 자체를 게임 중립 서치 레이어에 이식 — 예: MCTS
평가 함수에 "연속 열린 라인 수" 같은 일반화된 위협 지표를 섞는 것)은 이미
game-specific 휴리스틱과 game-neutral 서치 레이어의 경계를 넘나드는
설계이므로 이번 라운드 범위 밖으로 명시 이연한다. 이 시점에서는 "이 예산대·
이 탐색 깊이에서는 구조적으로 즉흥 설계 LLM 봇이 이긴다"는 결론 자체가
유효한 최종 결과다.
...
- 하스스톤 v2·오목 v4의 3열 벤치마크 재실행(현 리더보드는 구 버전 기준),
  오목 C열(vs Opus봇) 역전은 §4.8 진단(N=100, 전술 프리체크 포함 4번째 접근도
  0%)으로 이번 라운드 최종 확정 — 이 예산대에서는 역전되지 않는다(정식 3열
  벤치마크는 미실행, 진단 head-to-head로 방향 확인 후 웨이브 자체를 생략).
