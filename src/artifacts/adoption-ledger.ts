...
const SCREENED_VERDICT = 'screened';
...
function renderEntryRow(entry: AdoptionEntry, tier: string): string {
  const stats = entry.tierStats[tier];
  const winRate = stats ? formatPercent(stats.pointWinRate) : '-';
  const scoreDiff = stats ? stats.pointScoreDiff.toFixed(3) : '-';
...
export class AdoptionLedger {
  ...
  renderStrategyHistoryMarkdown(): string {
    ...
    for (const record of this.records) {
      ...
      lines.push(...renderVerdictSection('채택 (adopted)', adopted));
      lines.push(...renderVerdictSection('근접 실패 (near-miss)', nearMiss));
      lines.push(...renderVerdictSection('실패/선별 (failed/screened)', failed));
...
function renderVerdictSection(title: string, entries: readonly AdoptionEntry[]): string[] {
  const lines = [`### ${title}`, ''];
  if (entries.length === 0) {
    lines.push('- (없음)', '');
    return lines;
  }
  lines.push('| flags | verdict | winRate | scoreDiff | drawRate | blocks |');
  lines.push('|---|---|---|---|---|---|');
  for (const entry of entries) {
    lines.push(renderEntryRow(entry, highestPassedTier(entry)));
