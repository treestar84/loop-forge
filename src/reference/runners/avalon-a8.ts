/**
 * avalon-a8 — the A8 domain-redesign card's first live wave for avalon
 * (docs/GAP-ANALYSIS-12.md §7, scratchpad/avalon-a8-first-adoption-design-
 * spec.md). Mirrors avalon.ts's structure (same 6-step shape: conformance ->
 * calibration -> load-or-create registry/ledger -> wave -> adoption ledger
 * -> promotion -> summary) but scoped to just the new `avalonSuspicionTracking`
 * flag against the existing v1 baseline (registry/ledger already exist from
 * avalon.ts's own prior run, which screened out/failed the original 3
 * strategySurface flags — this file only appends a new wave, it never
 * touches that history).
 *
 * Fresh seed ranges (verified non-overlapping with every literal seed/bank
 * value in avalon.ts and avalon-benchmark.ts, the only two pre-existing
 * avalon runner files — grepped before picking these; avalon.ts occupies
 * screenProbe seeds 1-3/botSeedBase 100, smoke bank 1-~90, prune bank
 * 1000-1029, holdout bank 2000-2029, calibration identitySeeds
 * 700_000-700_099/calibrate-seed 42/measure-seed 600_000; avalon-benchmark.ts
 * occupies seeds 50_000-51_999/botSeedBases 800_001-800_003):
 *   - screenProbe: seeds 900_001-900_003, botSeedBase 900_100.
 *   - calibration identitySeeds: 904_000-904_099, calibrate-seed 43,
 *     measure-seed 905_000.
 *   - wave smoke/prune/holdout banks: 910_000+/920_000+/930_000+.
 *
...
const CANDIDATE_FLAG = 'avalonSuspicionTracking';
...
function main(): void {
  ...
  console.log(`=== avalon-a8 runner (rootDir=${rootDir}) ===`);

  console.log('1) G-Score conformance (re-check after adding avalonSuspicionTracking)');
  ...
  console.log('1.5) 캘리브레이션 — noise floor 기반 블록 수 산정 (fresh seeds, no overlap with avalon.ts)');
  const identitySeeds = Array.from({ length: 100 }, (_, i) => 904_000 + i);
  const noiseFloor = measureNoiseFloor(adapter, avalonAdapter.baselines.heuristic, identitySeeds, 905_000, {
    iterations: 2000,
  ...
  if (v1 === undefined) {
    throw new Error('avalon-a8 runner: v1 baseline missing — run avalon.ts first');
  }
  console.log(`   regression baseline: v1 (flags=[${v1.flags.join(', ')}])`);

  console.log('3) wave — single candidate avalonSuspicionTracking vs v1, screen→smoke→prune→holdout');
  ...
  waveLedger.reserve({
    bankId: 'avalon-a8-smoke',
    range: { start: 910_000, end: 910_000 + smokeMaxBlocks - 1 },
    purpose: 'smoke',
    reservedAt,
  });
  waveLedger.reserve({
    bankId: 'avalon-a8-prune',
    range: { start: 920_000, end: 920_000 + clampedBlocks - 1 },
    purpose: 'prune',
    reservedAt,
  });
  waveLedger.reserve({
    bankId: 'avalon-a8-holdout',
  ...
  const waveConfig = assembleWaveConfig(adapter, {
    ...
    tiers: {
      ...
      prune: { bankId: 'avalon-a8-prune', blocks: clampedBlocks },
      holdout: { bankId: 'avalon-a8-holdout', blocks: clampedBlocks },
    },
    screenProbe: { seeds: [900_001, 900_002, 900_003], botSeedBase: 900_100 },
  });
  ...
  if (adoptedFlags.length === 0) {
    ...
    if (currentLatest === undefined) {
      throw new Error('avalon-a8 runner: registry has no latest baseline before promotion step');
    }
  ...
  console.log('=== avalon-a8 runner complete ===');
}
