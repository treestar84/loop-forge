/**
 * catan-a8 — the A8 domain-redesign card's first live wave for catan
 * (docs/GAP-ANALYSIS-12.md §7, scratchpad/catan-a8-first-adoption-design-
 * spec.md). Mirrors avalon-a8.ts's structure (same shape: conformance ->
 * throughput check -> load-or-create registry/ledger -> wave -> adoption
 * ledger -> promotion -> summary) but scoped to just the new
 * `catanScarcityWeightedTrade` flag against the existing v1 baseline
 * (registry/ledger already exist from catan.ts's own prior run, which
 * screened out/failed all 3 original strategySurface flags — this file only
 * appends a new wave, it never touches that history).
 *
 * Fresh seed ranges (verified non-overlapping with every literal seed/bank
 * value in catan.ts and catan-benchmark.ts, the only two pre-existing catan
 * runner files — grepped before picking these; catan.ts occupies screenProbe
 * seeds 1-3/botSeedBase 100, calibration identitySeeds 700_000-700_059/
 * botSeedBase 600_000, wave-B smoke bank ~1-45/prune bank 1000+/holdout bank
 * 2000+, fieldMix bot-identity seeds 900_000-900_002; catan-benchmark.ts
 * occupies seeds 50_000-51_999 (default N=2000) /botSeedBases
 * 800_001-800_003):
 *   - screenProbe: seeds 911_001-911_003, botSeedBase 911_100.
 *   - calibration identitySeeds: 914_000-914_099, calibrate-seed 44,
 *     measure-seed 915_000.
 *   - throughput check: seeds 916_000-916_019 (20), botSeedBase 916_100
 *     (candidate run) / 916_200 (base run).
 *   - wave smoke/prune/holdout banks: 920_000+/930_000+/940_000+.
 *
...
const GAME_ID = 'catan';
const WAVE_ID = 'a8-wave-1';
const CANDIDATE_FLAG = 'catanScarcityWeightedTrade';
...
function main(): void {
  ...
  console.log(`=== catan-a8 runner (rootDir=${rootDir}) ===`);

  console.log('1) G-Score conformance (re-check after adding catanScarcityWeightedTrade — C6 in particular)');
  // Same c3SampleStates override catan.ts's own runner uses (initial
  ...
  console.log('1.5) C4 처리량 확인 (5배 한도 점검) — candidate vs base(heuristic), fresh seeds 916000-916019');
  const throughputSeeds = Array.from({ length: 20 }, (_, i) => 916_000 + i);
  ...
  runHeadToHead(adapter, catanAdapter.baselines.heuristic, catanAdapter.baselines.heuristic, throughputSeeds, 916_200);
  const baseElapsedMs = Date.now() - tBase0;
  const tCand0 = Date.now();
  runHeadToHead(adapter, candidateFactory, catanAdapter.baselines.heuristic, throughputSeeds, 916_100);
  ...
  console.log(`   catanScarcityWeightedTrade vs heuristic: ${candidateElapsedMs}ms`);
  console.log(`   비율: ${throughputRatio.toFixed(2)}x (5배 한도 ${throughputRatio <= 5 ? '통과' : '초과'})`);

  console.log('2) load registry/ledger from runs/catan/ (already exist from catan.ts prior run)');
  ...
  if (v1 === undefined) {
    throw new Error('catan-a8 runner: v1 baseline missing — run catan.ts first');
  }
  console.log(`   regression baseline: v1 (flags=[${v1.flags.join(', ')}])`);

  console.log('2.5) 캘리브레이션 — noise floor 기반 블록 수 산정 (fresh seeds, no overlap with catan.ts)');
  const identitySeeds = Array.from({ length: 100 }, (_, i) => 914_000 + i);
  const noiseFloor = measureNoiseFloor(adapter, catanAdapter.baselines.heuristic, identitySeeds, 915_000, {
    iterations: 2000,
  ...
  console.log('3) wave — single candidate catanScarcityWeightedTrade vs v1, screen→smoke→prune→holdout');
  ...
  waveLedger.reserve({
    bankId: 'catan-a8-smoke',
    range: { start: 920_000, end: 920_000 + smokeMaxBlocks - 1 },
    purpose: 'smoke',
    reservedAt,
  });
  waveLedger.reserve({
    bankId: 'catan-a8-prune',
    range: { start: 930_000, end: 930_000 + clampedBlocks - 1 },
    purpose: 'prune',
    reservedAt,
  });
  waveLedger.reserve({
    bankId: 'catan-a8-holdout',
    range: { start: 940_000, end: 940_000 + clampedBlocks - 1 },
    purpose: 'holdout',
    reservedAt,
  });
  ...
  const waveConfig = assembleWaveConfig(adapter, {
    ...
    tiers: {
      ...
      prune: { bankId: 'catan-a8-prune', blocks: clampedBlocks },
      holdout: { bankId: 'catan-a8-holdout', blocks: clampedBlocks },
    },
    screenProbe: { seeds: [911_001, 911_002, 911_003], botSeedBase: 911_100 },
  });
  ...
  writeFileSync(join(rootDir, 'runs', GAME_ID, 'near-miss-a8.json'), JSON.stringify(nearMiss, null, 2));
  console.log(`   저장: runs/${GAME_ID}/near-miss-a8.json`);
  ...
  if (adoptedFlags.length === 0) {
    ...
    if (currentLatest === undefined) {
      throw new Error('catan-a8 runner: registry has no latest baseline before promotion step');
    }
  ...
  console.log('=== catan-a8 runner complete ===');
}
