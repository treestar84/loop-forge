/**
 * dominion-benchmark — the 3-column win-rate benchmark of
 * docs/BENCHMARK-EXPERIMENT.md, run for dominion. This is NOT an onboarding
 * runner (that is reference/runners/dominion.ts, which scores conformance and
 ...
 *   C. Opus 설계봇      vs 루프포지봇
 *
 * "Opus 설계봇" = reference/experiments/dominion-opus-bot.ts, a one-shot LLM
...
const BOT_SEED_BASE = { A: 710_001, B: 720_002, C: 730_003 } as const;
...
function main(): void {
  ...
  const baseline = dominionAdapter.baselines.heuristic;
  const resolved = resolveLoopForgeFlags(rootDir);
  const loopForgeBot = composeBot(adapter, resolved.flags);

  console.log(`=== dominion 3-column benchmark (N=${n} seeds/column) ===`);
  ...
  const jsonPayload = {
    ...
    notes: [
      ...
      'Win rates across different games are NOT comparable (different baselines.heuristic strength); only A vs B vs C within dominion are.',
    ],
  };
...
- 승률은 게임 간 비교 불가(게임마다 \`baselines.heuristic\` 강함이 다름). 도미니언 내부에서 A·B·C 3개를 함께 보는 것이 실험 단위.
