...
function main(): void {
  ...
  /**
   * 14) ismcts-wave-2 (docs/FIX-BACKLOG.md P6, docs/GAP-ANALYSIS-8.md §2).
   * ismcts-wave-1 above was screened purely on the fixed cross-game
   * DEFAULT_CRITERIA.minScoreDiff=5: prune winRate=0.900 (CI 0.80-1.00)
   * cleared minWinRate comfortably, but pointScoreDiff=3.67 fell short of a
   * threshold that has no relationship to splendor's own score scale. P6
   * derives a game-calibrated threshold instead — 2 x the identity-self-play
   * scoreDiffStdDev already measured in step 2's `noiseFloor` (the noise
   * floor any real margin must clear to be distinguishable from noise,
   * INTERPRETATION.md §1) — and passes it through `assembleWaveConfig`'s
   * `calibration` argument so `deriveBlueprint`'s `recommendedMinScoreDiff`
   * becomes this wave's `criteria.minScoreDiff`.
   *
   * Same candidate, same logic as ismcts-wave-1 (`ismctsAdapter` /
   * `SPLENDOR_ISMCTS_S128_HR_FLAG`, unchanged) — only the gate criteria
   * differ, so this reuses the existing flag name rather than minting a new
   * one (§6.1's new-name rule governs behaviorally distinct candidates, not
   * a criteria change on the same candidate). New seed banks
   * (splendor-ismcts2-*, ranges 40000-43019) do not overlap ismcts-wave-1's
   * 30000-33019 range or any splendor-runner-* range.
   */
  console.log('14) IS-MCTS 후보 웨이브 2 (ismcts-wave-2, P6: 점수차 임계 캘리브레이션 파생)');
  ...
  if (ismcts2Latest === undefined) {
    throw new Error('splendor runner: registry has no latest baseline before ismcts-wave-2');
  }
  ...
  const ismcts2WaveConfig = assembleWaveConfig(
    ismctsAdapter,
    {
      ...
      tiers: {
        ...
        prune: { bankId: 'splendor-ismcts2-prune', blocks: ismcts2PruneBlocks },
        holdout: { bankId: 'splendor-ismcts2-holdout', blocks: ismcts2HoldoutBlocks },
        regression: { bankId: 'splendor-ismcts2-regression', blocks: ismcts2RegressionBlocks },
      },
      screenProbe: { seeds: [1, 2, 3], botSeedBase: 100 },
    },
  ...
  console.log('15) ismcts-wave-2 adoption ledger 기록');
  ...
  console.log('15.5) ismcts-wave-2 near-miss 후보 추출');
  ...
  console.log('15.6) ismcts-wave-2 채택 플래그 registry 승격');
  ...
  if (ismcts2AdoptedFlags.length === 0) {
    ...
    const currentLatest = registry.latest();
    if (currentLatest === undefined) {
      throw new Error('splendor runner: registry has no latest baseline before ismcts-wave-2 promotion step');
    }
    const lineage = registry.lineage(currentLatest.version);
  ...
  console.log('16) persist registry/ledger (ismcts-wave-2 반영)');
  saveRegistry(rootDir, GAME_ID, registry);
  saveLedger(rootDir, GAME_ID, ledger);
  console.log(`   anchors=${registry.listAnchors().length} adoptionRecords=${ledger.all().length}`);

  console.log('17) game-summary 재렌더 (ismcts-wave-2 반영)');
