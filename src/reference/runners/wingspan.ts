 ...
 */
const SOURCE_FILES = [
  join(__dirname, '..', 'wingspan.ts'),
  join(__dirname, '..', '..', 'search', 'mcts.ts'),
  join(__dirname, '..', '..', 'search', 'ismcts.ts'),
  join(__dirname, 'shared', 'wingspan-ismcts-flag.ts'),
...
function main(): void {
   ...
   * DEFAULT_CRITERIA.minScoreDiff=5: prune winRate=**1.000** cleared
   * minWinRate outright, but pointScoreDiff=4.07 still fell short of a
   * threshold with no relationship to wingspan's own score scale. P6 derives
   ...
   * (`ismctsAdapter` / `WINGSPAN_ISMCTS_S256_HR_FLAG`, unchanged) — only the
   * gate criteria differ. New seed banks (wingspan-ismcts2-*, ranges
   * 40000-43019) do not overlap ismcts-wave-1's 30000-33019 range or any
   * wingspan-runner-* range.
   */
  ...
  if (ismcts2Latest === undefined) {
    throw new Error('wingspan runner: registry has no latest baseline before ismcts-wave-2');
  }
  ...
  const ismcts2WaveConfig = assembleWaveConfig(
    ismctsAdapter,
    {
      ...
      tiers: {
        ...
        prune: { bankId: 'wingspan-ismcts2-prune', blocks: ismcts2PruneBlocks },
        holdout: { bankId: 'wingspan-ismcts2-holdout', blocks: ismcts2HoldoutBlocks },
        regression: { bankId: 'wingspan-ismcts2-regression', blocks: ismcts2RegressionBlocks },
      },
      screenProbe: { seeds: [1, 2, 3], botSeedBase: 100 },
    },
  ...
  if (ismcts2AdoptedFlags.length === 0) {
    ...
    const currentLatest = registry.latest();
    if (currentLatest === undefined) {
      throw new Error('wingspan runner: registry has no latest baseline before ismcts-wave-2 promotion step');
    }
    const lineage = registry.lineage(currentLatest.version);
